...
class PatchEmbed2D(nn.Module):
    def __init__(self, patch_size=4, in_chans=17, embed_dim=32, norm_layer=None, **kwargs):
        super().__init__()
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=17, depths=[2, 2, 9, 2], 
                 dims=[32, 64, 128, 256], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        ## Here, four layers are written, but only the first three layers are actually in use
        super().__init__()
...
class UFoldXall(nn.Module):
    ...
    def forward(self, x):
        # Encoding layers
        if x.shape[2]>600:  ## 0-1 Dynamic gating mechanism
            alpha=0
        else:
            alpha=1
        self.alpha=alpha
        ...
        d3 = self.alpha * d3_unet + (1 - self.alpha) * d3_vssm
